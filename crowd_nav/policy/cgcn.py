...
class ValueNetwork(nn.Module):
    def __init__(self, input_dim, self_state_dim, num_layer, X_dim, wr_dims, wh_dims, final_state_dim,
        ...
        self.value_net = mlp(final_state_dim + wr_dims[-1], planning_dims)
    ...
    def forward(self, state_input):
        ...
        # compute matrix A
        normalized_A = self.compute_similarity_matrix(human_state_embedings)
        ...
        if self.num_layer == 0:
            ...
            h1 = relu(torch.matmul(torch.matmul(normalized_A, human_state_embedings ), self.w1))
            u = torch.mean(h1, 1)
        else:
            # compute h1 and h2
            if not self.skip_connection:
                ...
                h1 = relu(torch.matmul(torch.matmul(normalized_A, human_state_embedings ), self.w1)) + human_state_embedings
            ...
            # u is the graph global attribute mentioned in https://arxiv.org/abs/1806.01261
            u = torch.mean(h2, 1)
        # do planning using only the final layer feature of the agent
        robot_state = torch.cat((u, self_state_embedings),dim = 1)
        value = self.value_net(robot_state)
        return value


class CGCN(MultiHumanRL):
    def __init__(self):
        super().__init__()
        self.name = 'CGCN'
